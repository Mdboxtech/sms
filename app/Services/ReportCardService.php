...
class ReportCardService
{
    ...
    public function generateReportCard(Student $student, Term $term)
    {
        ...
        $reportCardSettings = $this->getReportCardSettings();
        if ($reportCardSettings['color_theme'] !== 'school_colors') {
            $this->applyThemeColors($appSettings, $reportCardSettings['color_theme']);
        }
    ...
    private function getReportCardSettings()
    {
        $settings = [
            'template' => \App\Models\Setting::getValue('report_card_template', 'classic'),
            'color_theme' => \App\Models\Setting::getValue('report_card_color_theme', 'school_colors'),
            'show_attendance' => \App\Models\Setting::getValue('report_card_show_attendance', true),
            'show_position' => \App\Models\Setting::getValue('report_card_show_position', true),
            'show_class_average' => \App\Models\Setting::getValue('report_card_show_class_average', true),
            'show_teacher_comment' => \App\Models\Setting::getValue('report_card_show_teacher_comment', true),
            'show_principal_comment' => \App\Models\Setting::getValue('report_card_show_principal_comment', true),
            'header_text' => \App\Models\Setting::getValue('report_card_header_text', 'STUDENT REPORT CARD'),
            'disclaimer' => \App\Models\Setting::getValue('report_card_disclaimer', 'This result is generated by the school management system and is subject to errors and omissions.'),
            'show_notable_attributes' => \App\Models\Setting::getValue('report_card_notable_attributes', false),
            'table_header_color' => \App\Models\Setting::getValue('report_card_table_header_color', '#1e40af'),
            'table_border_color' => \App\Models\Setting::getValue('report_card_table_border_color', '#e2e8f0'),
        ];

        return $settings;
    }

    private function applyThemeColors(&$appSettings, $theme)
    {
        $colors = [
            'blue' => ['primary' => '#1e40af', 'secondary' => '#3b82f6'],
            'green' => ['primary' => '#15803d', 'secondary' => '#22c55e'],
            'red' => ['primary' => '#b91c1c', 'secondary' => '#ef4444'],
            'grayscale' => ['primary' => '#374151', 'secondary' => '#9ca3af'],
        ];

        if (isset($colors[$theme])) {
            $appSettings['school_primary_color'] = $colors[$theme]['primary'];
            $appSettings['school_secondary_color'] = $colors[$theme]['secondary'];
        }
    }
}
