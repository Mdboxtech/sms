...
export default function TeacherResults({ auth, teacher, results }) {
    ...
    useEffect(() => {
        if (results && results.length > 0) {
            ...
            results.forEach(result => {
                const subjectId = result.subject?.id || 'unknown';
                ...
                grouped[subjectId].results.push(result);
            });
    ...
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<PageHeader>Teacher Results: {teacher?.user?.name}</PageHeader>}
        >
            <Head title={`${teacher?.user?.name} Results - SMS`} />
            ...
            <div className="py-12">
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    <div className="mb-6 flex justify-between items-center">
                        ...
                        <Link
                            href={route('admin.results.export', { teacher_id: teacher?.id })}
                    ...
                    <div className="bg-white shadow overflow-hidden sm:rounded-lg mb-6">
                        <div className="px-4 py-5 sm:px-6">
                            ...
                            <p className="mt-1 max-w-2xl text-sm text-gray-500">
                                Personal details and result statistics
                            </p>
                        </div>
                        <div className="border-t border-gray-200">
                            <dl>
                                ...
                                <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                    <dt className="text-sm font-medium text-gray-500">Total Results Recorded</dt>
                                    <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                                ...
                                <div className="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                    ...
                                    <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                                        {statistics.lowestScore.toFixed(1)}% - {statistics.highestScore.toFixed(1)}%
                                    </dd>
                                </div>
                            </dl>
                        </div>
                    ...
                    {Object.values(resultsBySubject).map(({ subject, results: subjectResults }) => (
                        <Card key={subject?.id || 'unknown'} className="mb-6">
                            <div className="p-6">
                                <div className="flex justify-between items-center mb-4">
                                    <div>
                                        ...
                                        <p className="text-sm text-gray-500">{subjectResults.length} results</p>
                                    </div>
                                    <Link
                                        href={route('admin.results.subject', { subject_id: subject?.id })}
                                                ...
                                                return (
                                                    <tr key={result.id}>
                                                        <td className="px-6 py-4 whitespace-nowrap">
                                                            ...
                                                            <div className="text-xs text-gray-500">{result.student?.admission_number}</div>
                                                        </td>
                                                        <td className="px-6 py-4 whitespace-nowrap">
                                                            <div className="text-sm text-gray-900">{result.student?.classroom?.name || 'Not assigned'}</div>
                                                        </td>
                                                        <td className="px-6 py-4 whitespace-nowrap">
                                                            <div className="text-sm text-gray-900">{result.term?.name}</div>
                                                            <div className="text-xs text-gray-500">{result.term?.academic_session?.name}</div>
                                                        </td>
                                                        <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    {Object.keys(resultsBySubject).length === 0 && (
                        <Card>
                            <div className="p-6 text-center">
                                <p className="text-gray-500">No results recorded by this teacher.</p>
                            </div>
                        </Card>
                    )}
                </div>
